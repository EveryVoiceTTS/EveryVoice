    ...
    def test_stats(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with (
                ...
                _, _, char_length_data, _ = preprocessor.compute_stats(
                    energy=False, pitch=False, char_length=True, phone_length=False
                )
                ...
                self.assertEqual(char_length_stats["std"], sqrt(200.5))
                self.assertEqual(char_length_stats["mean"], 105)

                # Create a preprocessor with multiple cpus
                preprocessor = Preprocessor(fp_config)
                preprocessor.preprocess(
                    output_path=lj_filelist, to_process=("audio", "text")
                )
