...
def test_synthesize(dummy_fp_path, dummy_vocoder_path):
    ...
    os.environ["PYTORCH_ENABLE_MPS_FALLBACK"] = "1"  # Fallback for running tests on Mac
...
def test_update_schemas(subtests):
...
def test_old_inspect_checkpoint():
    result = CliRunner().invoke(
...
def test_inspect_checkpoint_help():
...
def test_inspect_good_vocoder_checkpoint(dummy_vocoder_path) -> None:
    result = CliRunner().invoke(app, ["checkpoint", "inspect", str(dummy_vocoder_path)])
    assert result.exit_code == 0
...
def test_export_and_inspect_generator(dummy_vocoder_path) -> None:
...
def test_preprocess_without_subcommand_shows_subcommands():
...
def test_preprocess_text_to_wav_help():
...
def test_expensive_imports_are_tucked_away():
...
def test_rename_speaker():
    with tempfile.TemporaryDirectory() as tmpdir_str:
        ...
        ckpt = {
            "hyper_parameters": {"speaker2id": {"old_speaker": 0, "another_speaker": 1}}
        }
        ...
        with mock.patch("torch.save", side_effect=mock_function_placeholder):
            ...
            assert result.exit_code == 0
            assert "Renamed speaker 'old_speaker' to 'new_speaker'." in result.output
            assert (
...
def test_rename_speaker_with_non_existing_speaker():
    with tempfile.TemporaryDirectory() as tmpdir_str:
        import torch
...
def test_rename_speaker_with_no_speakers():
    with tempfile.TemporaryDirectory() as tmpdir_str:
        import torch
...
def test_save_configuration_to_log_dir():
