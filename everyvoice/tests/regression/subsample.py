...
def subsample(
    ...
    duration: int = typer.Option(
        ..., "-d", "--duration", help="Ideal duration of subsample in minutes."
    ),
    format: Format = typer.Option(
        ..., "-f", "--format", help="File format (psv, csv, tsv and festival)."
    ),
    ...
    # Perform validation
    if format == Format.FESTIVAL and speakerid:
    ...
    # Read metadata file
    if format == Format.FESTIVAL:
        metadata = read_festival(metadata_path)
    else:
        separators = {Format.PSV: "|", Format.TSV: "\t", Format.CSV: ","}
    ...
    for i, dict in enumerate(metadata):
        if speakerid and speakerid != dict["speaker"]:  # Filter for correct speaker
            continue
        try:
            filepath = join(wavs_path, dict["basename"] + ".wav")
