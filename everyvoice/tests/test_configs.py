...
class ConfigTest(TestCase):
    ...
    def test_config_save_dirs(self):
        with TemporaryDirectory(prefix="test_config_save_dirs") as tempdir:
            ...
            assert (tempdir / preprocessing_config.save_dir).exists()
    ...
    def test_config_partial(self):
        with (
            ...
            with self.assertRaises(exceptions.InvalidConfiguration):
                with NamedTemporaryFile(prefix="test", mode="w", suffix=".yaml") as tf:
                    tf.write(" ")
                    tf.flush()
            ...
            assert isinstance(fp_config, FeaturePredictionConfig)
            # Vocoder Config
            _writer_helper(
    ...
    def test_config_partial_override(self):
        ...
        with NamedTemporaryFile(
            ...
            assert config.audio.min_audio_length == 1.0
            # override with dict
            with mute_logger("everyvoice.config.utils"):
                config = PreprocessingConfig(
                    path_to_audio_config_file=tf.name, audio={"max_audio_length": 1.0}
                )
            assert config.audio.max_audio_length == 1.0
    ...
    def test_string_to_callable(self):
        ...
        assert config.text.cleaners == [lower]
    ...
    def test_call_sub_dir(self):
        ...
        assert "sub_dir" not in serialized_config
    ...
    def test_change_with_indices(self):
        ...
        with (
            ...
            self.assertEqual(
                config.preprocessing.source_data[0].filelist,
                Path("/foo/bar/filelist.psv"),
            )
...
class LoadConfigTest(TestCase):
    ...
    def test_preprocessing_config(self):
        ...
        with config_path.open("r", encoding="utf8") as f:
            ...
            assert len(pre_test["source_data"]) == 1
        ...
        assert config.dataset == self.DATASET_NAME
        self.validate_config_path(config.save_dir)
        assert len(config.source_data) == 1
    ...
    def test_vocoder_config(self):
        ...
        # print(config.model_dump_json(indent=2))
        assert isinstance(config, VocoderConfig)
        assert config.preprocessing.dataset == self.DATASET_NAME
    ...
    #        )
    #        assert isinstance(aligner_config, AlignerConfig)
    ...
    #        assert isinstance(config, AlignerConfig)
    #        assert config.preprocessing.dataset == self.DATASET_NAME
